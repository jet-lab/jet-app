...
import { InstructionAndSigner, sendAllTransactions, sendTransaction, transactionErrorToString } from '../util/programUtil';
...
export const useJetV1 = () => {
  ...
  // Deposit
  const deposit = async (abbrev: string, lamports: BN)
  ...
  // Withdraw
  const withdraw = async (abbrev: string, amount: Amount)
  : Promise<[res: TxnResponse, txid: string[]]> => {
  if (!user.assets || !publicKey || !program) {
    return [TxnResponse.Failed, []];
  }
  ...
  // Borrow
 const borrow = async (abbrev: string, amount: Amount)
  : Promise<[res: TxnResponse, txid: string[]]> => {
  if (!user.assets || !publicKey || !program) {
    return [TxnResponse.Failed, []];
  }
  ...
  // Repay
  const repay = async (abbrev: string, amount: Amount)
  : Promise<[res: TxnResponse, txid: string[]]> => {
  if (!user.assets || !publicKey || !program) {
    return [TxnResponse.Failed, []];
  }
  ...
  * until it can be fully refreshed once more. */
  ...
  // Faucet
  const airdrop = async (abbrev: string, lamports: BN)
  : Promise<[res: TxnResponse, txid: string[]]> => {
  ...
  if (reserve.tokenMintPubkey.equals(NATIVE_MINT)) {
    ...
    const mintToIx = Token.createMintToInstruction(TOKEN_PROGRAM_ID, reserve.tokenMintPubkey, asset.walletTokenPubkey, publicKey, [], new u64(lamports.toArray()));
    ix.push(mintToIx);
